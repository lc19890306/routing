 ...
 * a packet is forwarded to the nearest neighbor in forwarding progress
 * It lacks odd symmetry, transitivity, strict order
 */
...
float weight(int u[], int v[], int d[]) {
  ...
  float prod = (v[0]-u[0])*(d[0]-u[0])+(v[1]-u[1])*(d[1]-u[1]);
  float dist_uv = dist(u, v);
  float dist_ud = dist(u, d);
  __ESBMC_assume(dist_uv*dist_ud > 0);
  float cos = prod;//(dist_uv*dist_ud);
  if (cos >= 0)
    return dist_uv;
  else return -dist_uv;
}
