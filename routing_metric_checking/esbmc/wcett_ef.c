...
float foo(float ETT[MAX], int x[MAX]) {
  int i;
  float max = 0;
  float ret = 0, ret2 = 0;
  int channel[MAX] = {0};
  ...
  ret2 = (1 - beta) * ret + beta * max;
  return ret2;
}

float bar(float a_ETT[MAX], int a_x[MAX], float b_ETT[MAX], int b_x[MAX]) {
  int i;
  float max = 0;//max_a = 0, max_b = 0;
  float ret = 0, ret2 = 0, ret3 = 0;
  int channel[MAX] = {0};
  for (i = 0; i < MAX; ++i) {
    ret += a_ETT[i];
    ret2 += b_ETT[i];
  ...
  ret3 = (1 - beta) * (ret + ret2) + beta * max;//((max_x > max_y) ? max_x : max_y);
  return ret3;
}

int main() {
  float a_ETT[MAX], b_ETT[MAX], c_ETT[MAX];
